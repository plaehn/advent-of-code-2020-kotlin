...
class JoltageDifferenceComputer(adapterJoltages: List<Int>) {
    ...
    fun computeJoltageDifferenceDistributionProduct(): Int =
            computeJoltageDifferences()
                    .groupingBy { it }
                    .eachCount()
                    .map { it.value }
                    .product()

    private fun computeJoltageDifferences(): List<Int> = sortedJoltages
    ...
    fun countDistinctValidArrangements(): Long {
        val chunks = chunkByEqualElements(computeJoltageDifferences().asSequence()).toList()

        val factors = chunks
                .map { if (it.first() == 1) fibonacci(it.count() + 2) - 1 else 1 }
                .map { it.toLong() }
